...
public class insertDB {
	...
	// 사용자 정보테이블에 값넣기
    public void addCustomerInfo(campDataBean member,String licensenumber) throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ...
        try {
            ...
            String sql = "insert into customer_info (customer_info,licensenumber,rentalnumber)\n"
                                        + "values(CUSTOMER_INFO_NUM.NEXTVAL,?,?)";
            pstmt = conn.prepareStatement(sql);

            pstmt.setString(1,licensenumber);
            pstmt.setInt(2, member.getRentalNumber());
